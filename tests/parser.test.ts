...
unarias
  ...
  / regla3* / (regla4?)*;
    ...
    it("Prueba para archivo de prueba NUEVO 1", () => {
        const input = readFileSync(join(__dirname, 'archivos_prueba', 'pruebaNueva1.txt'), 'utf-8')
        const result = Parser.parse(input);
        expect(result).toBeDefined();
    });

    it("Prueba para archivo de prueba NUEVO 2", () => {
        const input = readFileSync(join(__dirname, 'archivos_prueba', 'pruebaNueva2.txt'), 'utf-8')
        const result = Parser.parse(input);
        expect(result).toBeDefined();
    });

    it("Prueba para archivo de prueba NUEVO 3", () => {
        const input = readFileSync(join(__dirname, 'archivos_prueba', 'pruebaNueva3.txt'), 'utf-8')
        const result = Parser.parse(input);
        expect(result).toBeDefined();
    });
    ...
    it('Deberia fallar al tratar de validar entrada incorrecta', () => {
        expect(() => Parser.parse("a = 'si' b = 'no'")).toThrow();
    });
});
